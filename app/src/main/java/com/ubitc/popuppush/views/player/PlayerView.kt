...
class PlayerView : StyledPlayerView, Player.Listener, MyView {
...
//            .createMediaSource(link)
        ...
        val newParameters = parameters.buildUpon().setMaxVideoBitrate(dts.parameters.maxVideoBitrate * 2).build() // 2 Mbps
        newParameters.buildUpon().setMaxVideoFrameRate(dts.parameters.maxVideoFrameRate * 2).build() // 2 Mbps
    ...
    override fun prepareView(media: MediaModel?, viewListener: ViewListener?) {
        ...
        createPlayer(createMediaItem(link!!))
    }
...
//            .createMediaSource(link)
        ...
        val l = object : Player.Listener {
            override fun onIsPlayingChanged(isPlaying: Boolean) {
                ...
                if (isPlaying) {
                    player?.removeListener(this)
                    player?.addListener(this@PlayerView)
                }
            }
        }
        player = ExoPlayer.Builder(context, rf)
            ...
            .build().apply {
                ...
                if (media?.isMuted == true)
                    volume = 0f
                addListener(l)
                setMediaItem(createMediaItem(media?.path!!), pos)
                prepare()
            }
...
//    }
        ...
        player?.addListener(object : Player.Listener {
            override fun onPlaybackStateChanged(state: Int) {
                if (state == Player.STATE_ENDED || state == Player.STATE_IDLE) {
                    param?.finished(this@PlayerView)
                }
            }
    ...
    override fun onPlayerStateChanged(playWhenReady: Boolean, playbackState: Int) {
        if (playbackState == Player.STATE_READY) {
            param?.viewIsReady(this@PlayerView)
